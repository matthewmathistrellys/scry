...
class ScryHookTests(unittest.TestCase):
    ...
    def _stack_cli_repo(self, base, with_dockerfile=False):
        """A repo whose config implies fly, mix, psql, gh — and docker only
        when with_dockerfile is set."""
        base.mkdir(parents=True, exist_ok=True)
        (base / ".env").write_text(
            "DATABASE_URL=postgres://u:p@ep-x.us-east-2.aws.neon.tech/db\n")
        app = base / "fly.toml"
        app.write_text('app = "engine"\nmin_machines_running = 0\n')
        (base / "mix.exs").write_text("defmodule M do\nend\n")
    ...
    def _shim_path(self, td, tools):
        """A PATH dir where exactly `tools` resolve — plus git and python3,
        which stack.sh itself needs — so the CLI check is deterministic no
        matter what the host has installed."""
        shim = Path(td) / "shim"
        shim.mkdir()
        for tool in list(tools) + ["git", "python3", "bash", "dirname"]:
            target = shim / tool
            if tool in ("git", "python3", "bash", "dirname"):
    ...
    def test_stack_names_the_clis_the_repo_implies_when_they_are_on_path(self):
        """An agent that is never told `fly` exists never asks (Tidewave rule):
        presence is stated, not left for the session to discover on demand."""
        with tempfile.TemporaryDirectory() as td:
            base = self._stack_cli_repo(Path(td) / "r")
            shim = self._shim_path(td, ["fly", "mix", "psql", "gh", "docker"])
            out = context(run_hook("stack.sh", base, {}, {"PATH": shim}))
            self.assertIn("Repo CLIs on PATH", out)
            self.assertIn("fly (deploys)", out)
            self.assertIn("mix (Elixir)", out)
            self.assertIn("psql (database client)", out)
            self.assertIn("gh (PRs/CI)", out)
            # Derived, never enumerated: no docker marker in the repo, so
            # docker is not checked and not named even though it resolves.
            self.assertNotIn("containers", out)
            self.assertNotIn("NOT on the PATH", out)

    def test_stack_flags_a_repo_implied_cli_missing_from_the_session_path(self):
        """The miss is the actionable half: found at session start, not
        mid-deploy. Phrased as the PATH fact it is — hooks run in a
        non-interactive environment, so "not installed" could be silently
        wrong about a login shell's rc-file PATH additions."""
        with tempfile.TemporaryDirectory() as td:
            base = self._stack_cli_repo(Path(td) / "r", with_dockerfile=True)
            shim = self._shim_path(td, ["fly", "mix", "psql", "gh"])
            out = context(run_hook("stack.sh", base, {}, {"PATH": shim}))
            self.assertIn("Repo CLIs on PATH", out)
            self.assertIn("fly (deploys)", out)
            self.assertIn("NOT on the PATH this session sees", out)
            self.assertIn("docker (containers)", out)

    def test_stack_cli_check_switches_off(self):
        with tempfile.TemporaryDirectory() as td:
            base = self._stack_cli_repo(Path(td) / "r")
            shim = self._shim_path(td, ["fly", "mix", "psql", "gh"])
            out = context(run_hook("stack.sh", base, {},
                                   {"PATH": shim, "SCRY_REPO_CLI_CHECK": "0"}))
            self.assertNotIn("Repo CLIs", out)

    def test_implied_clis_checks_only_what_the_repo_implies(self):
        """Unit-level: the roster is derived from found config, so a repo that
        implies nothing yields no checks at all."""
        sys.path.insert(0, str(ROOT / "scanners"))
        import stack as stack_mod
        calls = []
        ...
        empty = {"fly_apps": [], "mix": 0, "python": False, "node": False,
                 "docker": False, "ci": False}
        self.assertEqual(stack_mod.implied_clis({}, empty, resolve), [])
        self.assertEqual(calls, [])
        facts = dict(empty, fly_apps=["engine"], ci=True)
        result = stack_mod.implied_clis({}, facts, resolve)
        self.assertEqual(result, [("fly", True, "deploys"), ("gh", True, "PRs/CI")])
        missing = stack_mod.implied_clis({}, facts, lambda t: None)
        self.assertEqual(missing, [("fly", False, "deploys"), ("gh", False, "PRs/CI")])
